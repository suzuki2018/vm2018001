...
typedef void (*method_handler_ptr)(Request& request, Response &response, long userdata);
...
class HttpEpollWatcher : public EpollSocketWatcher {
	...
	void add_mapping(std::string path, method_handler_ptr handler, long userdata, HttpMethod method = GET_METHOD);
