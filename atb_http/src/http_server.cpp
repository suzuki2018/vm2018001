...
void HttpServer::add_mapping(std::string path, method_handler_ptr handler, long userdata,  HttpMethod method){
	http_handler.add_mapping(path, handler, userdata, method);
}
...
void HttpEpollWatcher::add_mapping(std::string path, method_handler_ptr handler, long userdata, HttpMethod method)
{
	Resource resource = {method, handler, NULL, userdata};
	resource_map[path] = resource;
}

void HttpEpollWatcher::add_mapping(std::string path, method_handler_ptr handler, HttpMethod method) {
    Resource resource = {method, handler, NULL,0};
    resource_map[path] = resource;
}

void HttpEpollWatcher::add_mapping(std::string path, json_handler_ptr handler, HttpMethod method) {
    Resource resource = {method, NULL, handler,0};
    resource_map[path] = resource;
}
...
int HttpEpollWatcher::handle_request(Request &req, Response &res) {
 ...
 res.set_head("Access-Control-Allow-Methods","POST,GET");  
res.set_head("Access-Control-Allow-Headers", "Cache-Control,X-Proxy-Authorization,X-Requested-With,Content-Type");
 ...
 std::string uri = req.get_request_uri();
    ...
    if (resource.json_ptr != NULL) {
        ...
        resource.handler_ptr(req, res,resource.userdata);
    }
